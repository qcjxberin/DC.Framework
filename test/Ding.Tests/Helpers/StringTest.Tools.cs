...
namespace Ding.Tests.Helpers {
    ...
    public partial class StringTest {
        ...
        public void TestJoin() {
            Assert.Equal( "1,2,3", Ding.Helpers.String.Join( new List<int> { 1, 2, 3 } ) );
            Assert.Equal( "'1','2','3'", Ding.Helpers.String.Join( new List<int> { 1, 2, 3 }, "'" ) );
            Assert.Equal( "123", Ding.Helpers.String.Join( new List<int> { 1, 2, 3 }, "", "" ) );
            Assert.Equal( "\"1\",\"2\",\"3\"", Ding.Helpers.String.Join( new List<int> { 1, 2, 3 }, "\"" ) );
            Assert.Equal( "1 2 3", Ding.Helpers.String.Join( new List<int> { 1, 2, 3 }, "", " " ) );
            Assert.Equal( "1;2;3", Ding.Helpers.String.Join( new List<int> { 1, 2, 3 }, "", ";" ) );
            Assert.Equal( "1,2,3", Ding.Helpers.String.Join( new List<string> { "1", "2", "3" } ) );
            Assert.Equal( "'1','2','3'", Ding.Helpers.String.Join( new List<string> { "1", "2", "3" }, "'" ) );
            ...
            Assert.Equal( "83B0233C-A24F-49FD-8083-1337209EBC9A,EAB523C6-2FE7-47BE-89D5-C6D440C3033A".ToLower(), Ding.Helpers.String.Join( list ) );
            Assert.Equal( "'83B0233C-A24F-49FD-8083-1337209EBC9A','EAB523C6-2FE7-47BE-89D5-C6D440C3033A'".ToLower(), Ding.Helpers.String.Join( list, "'" ) );
        }
