        ...
        private async Task WriteViewToFileAsync( PageHandlerSelectedContext context ) {
            try {
                ...
                if ( string.IsNullOrWhiteSpace( relativePath ) )
                    return;
                var path = Ding.Helpers.Common.GetPhysicalPath( relativePath );
        ...
        protected virtual string GetPath( PageHandlerSelectedContext context ) {
            ...
            if( context.ActionDescriptor is CompiledPageActionDescriptor pageActionDescriptor ) {
                ...
                for( var i = 0; i < paths.Length; i++ ) {
                    var path = paths[i];                    
                    var name = Ding.Helpers.String.SplitWordGroup( path );
                    if (name == "pages" && i == 0)
                    {
                    ...
                    if( i == paths.Length - 1 ) {
                        name = Ding.Helpers.String.SplitWordGroup( path.RemoveEnd( ".cshtml" ) );
