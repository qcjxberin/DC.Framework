            ...
            if (defaultAuthenticate != null)
            {
                ...
                if (result?.Principal != null)
                {
                    ...
                    foreach (var item in permisssionGroup)
                    {
                        try
                        {
                            if (Regex.Match(questUrl, item.Key?.SafeString().ToLower())?.Value == questUrl)
                            {
                                isMatchUrl = true;
                                break;
                            }
                    ...
                    if (isMatchUrl)
                    {
                        ...
                        foreach (var item in permisssionRoles)
                        {
                            try
                            {
                                if (Regex.Match(questUrl, item.Url?.SafeString().ToLower())?.Value == questUrl)
                                {
                                    isMatchRole = true;
                                    break;
                                }
                ...
                else
                {
                    ...
                    {
                        httpContext.Response.Redirect(requirement.H5LoginPath + $"?ReturnUrl={Web.UrlEncode(httpContext.Request.GetDisplayUrl())}");
                    }
                    else
                    {
                        httpContext.Response.Redirect(requirement.PcLoginPath + $"?ReturnUrl={Web.UrlEncode(httpContext.Request.GetDisplayUrl())}");
                    }
                    context.Succeed(requirement);
                    return;
                }
